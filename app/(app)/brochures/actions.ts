...
export async function createBrochure(formData: FormData) {
  ...
  if (file instanceof File && file.size > 0) {
    if (file.size > MAX_BYTES) {
      return { ok: false, message: "File is too large — the limit is 20 MB." };
    }
    const ext = ALLOWED_TYPES[file.type];
    if (!ext) return { ok: false, message: "Brochure file must be a PDF, JPEG, PNG, or WEBP." };
  ...
  if (!url) return { ok: false, message: "Upload a file or paste a link." };
